...
Architecture:
  ...
  - RealDataSource: stub for hooking in actual capture + model inference
  - CaptureWorker: background thread that pushes results to a queue
  - Streamlit UI: polls queue and updates all placeholders

To switch from fake → real data:
  1. Implement RealDataSource.capture_loop() with scapy AsyncSniffer
  2. Point feature extraction at your extract_features.py logic
  3. Load your trained model and call model.predict_proba()
...
# ── FEATURE NAMES (mirrors what extract_features.py should produce) ──────────
FEATURE_NAMES = [
    "total_packets",
    "total_bytes",
    "outgoing_packets",
...
class FakeDataSource:
    ...
    """
    # Per-site traffic "fingerprint" parameters
    SITE_PROFILES = {
    ...
    def generate_packet_window(self) -> list[PacketRecord]:
        ...
        for _ in range(WINDOW_SIZE):
            ...
            size = min(size, 1500)
            iat = max(0.1, np.random.exponential(10))  # ms
            t += iat / 1000.0
    ...
    def extract_features(self, packets: list[PacketRecord]) -> dict:
        ...
        # Simple burst detection: gap > 50ms starts a new burst
    ...
    def fake_model_predict(self, features: dict, true_site: str) -> tuple[str, float, dict]:
        """
        Fake model that uses feature similarity to produce plausible probabilities.
        When defense is active, predictions are intentionally noisy.
        """
        defense = bool(self.defense_active_fn())
        probs = {}

        if defense:
            # Defense active: uniform-ish noise, low confidence
            raw = {s: random.uniform(0.05, 0.25) for s in MONITORED_SITES}
        else:
            # Score each site by how well features match its profile
            raw = {}
